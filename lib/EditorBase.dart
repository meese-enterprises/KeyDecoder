...
class ShapeNotifier extends ChangeNotifier {
  ...
  void rescaleDelta(double scaleDelta) {
    all?.forEach((element) {
      ...
      if (element is Circle) element.radius = element.radius / scaleDelta;
...
class EditorBaseState extends State<EditorBase> {
  ...
  double get scaleFactor =>
      imageRenderBox.getTransformTo(ancestor).getColumn(0).length;
  ...
  Offset globalToLocal(Offset o) {
    if (imageRenderBox == null) return Offset.zero;

    Offset out = (widget.portrait) ? Offset(o.dy, o.dx) : Offset(o.dx, o.dy);

    Offset globalZero =
        imageRenderBox.localToGlobal(Offset.zero, ancestor: ancestor);
    Offset localZero =
        imageRenderBox.globalToLocal(Offset.zero, ancestor: ancestor);
    ...
    if (_fromPortrait) {
      if (widget.portrait) {
        ...
        localZero = invertOffset(
            imageRenderBox.globalToLocal(globalZero, ancestor: ancestor));
        if (matrix.isIdentity()) out += globalZero;
        out = imageRenderBox.globalToLocal(out, ancestor: ancestor);
      }
    } else {
      if (widget.portrait) {
  ...
  void _onMoveStart(int pointer, Offset localPos, Offset position) {
    if (localPos == null || imageRenderBox == null) return;
    ...
    for (int i = 0; i < _draggable.length; i++) {
      ...
      if (d > elt.radius * EditorBase.touchSizeFactor) continue;
    ...
    if (closest < 0) {
      if (widget.onTap != null) widget.onTap(transformed);
      return;
    }
  ...
  void _onMoveUpdate(
    ...
    if (_drag == null) {
      ...
      if (widget.portrait) localDelta = Offset(-localDelta.dy, localDelta.dx);
  ...
  Widget build(BuildContext context) {
    if (_mediaQuery == null) {
      ...
      if ((_mediaQuerySize.width > _mediaQuerySize.height &&
              _imageSize.width > _imageSize.height) ||
          (_mediaQuerySize.width < _mediaQuerySize.height &&
              _imageSize.width < _imageSize.height)) {
                              ...
                              Consumer<ShapeNotifier>(
                                builder: (painterContext, value, child) {
                                  return CustomPaint(
                                    key: painterKey,
                                    size: _imageSize,
                                    painter: EditorPainter(painterContext,
                                        shapes: value.all),
                                  );
                                },
                              )
  ...
  void onScaleUpdate(Offset focalPoint, double scale, double rotation) {
    ...
    if (rotation != 0.0) {
      if (rotationUpdater.value.isNaN) {
        ...
        if (widget.onRotation != null) widget.onRotation(rotationDelta);
      }
    }
