  ...
  static Route<dynamic> route(Object args) {
    return MaterialPageRoute(
      builder: (BuildContext context) {
        if (args is File) return CropPage.fromFile(args);
        throw ArgumentError;
      },
    );
  }
...
class CropPageState extends State<CropPage> {
  ...
  void initState() {
    ...
    if (imageSize.width < imageSize.height && !_portrait) {
      _portrait = true;
    } else if (_portrait) imageSize = imageSize.flipped;
  ...
  void addLine() {
    ...
    Line newLine = Line(
        _editorBaseState
            .globalToLocal(Offset(longestSide * 1 / 4, shortestSide * 1 / 2)),
        _editorBaseState
            .globalToLocal(Offset(longestSide * 3 / 4, shortestSide * 1 / 2)),
        _cropPaint);
    ...
    if (shapeNotifier.draggableShapes.length >= 2) {
      ...
      shapeNotifier.draggableShapes.add(
          Circle(newLine.a, EditorBase.touchRadius / scaleFactor, _cropPaint));
      shapeNotifier.draggableShapes.add(
          Circle(newLine.b, EditorBase.touchRadius / scaleFactor, _cropPaint));
    }
  ...
  void removeLine() {
    if (shapeNotifier.lineShapes.isEmpty) return;
    ...
    if (shapeNotifier.draggableShapes.length >= 2) {
      ...
      shapeNotifier.draggableShapes.clear();
      shapeNotifier.draggableShapes.add(
          Circle(lastLine.a, EditorBase.touchRadius / scaleFactor, _cropPaint));
      shapeNotifier.draggableShapes.add(
          Circle(lastLine.b, EditorBase.touchRadius / scaleFactor, _cropPaint));
    }
  ...
  Future cropImage() async {
    ...
    ui.Rect bounds = Offset(-pad, -pad) &
        Size(imageSize.width + 2 * pad, imageSize.height + 2 * pad);

    for (var i = 0; i < 3; i++) {
      intersections
          .addAll(lines[i].xtersectList(lines.sublist(i + 1), bounds: bounds));
    }
    ...
    double d1 = (
      (dinter.topLeft - dinter.topRight).distanceSquared +
      (dinter.bottomRight - dinter.bottomLeft).distanceSquared
    ) / 2.0;
    double d2 = (
      (dinter.topRight - dinter.bottomRight).distanceSquared +
      (dinter.bottomLeft - dinter.topLeft).distanceSquared
    ) / 2.0;
    ...
    final resultPath = p.join(
      widget.imageFile.parent.path,
      "crop_" + p.basenameWithoutExtension(widget.imageFile.path) + ".png"
    );

    int res = await HomographyWarp.homographyWarp(
      dinter,
      widget.imageFile.path,
      resultPath
    );

    if (res < 0) throw Exception("homographyWarp failed");
  ...
  int compare(Offset p1, Offset p2, Offset ref) {
    ...
    if (o == 0)
      return ((ref - p2).distanceSquared >= (ref - p1).distanceSquared)
          ? -1
          : 1;
