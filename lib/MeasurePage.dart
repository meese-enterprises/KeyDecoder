  ...
  static Route<dynamic> route(Object args) {
    return MaterialPageRoute(
      builder: (BuildContext context) {
        if (args is Project) return MeasurePage(args);
        throw ArgumentError;
      },
    );
  }
...
class MeasurePageState extends State<MeasurePage> {
  ...
  EditorBaseState get _editorBaseState =>
      (editorKey.currentState as EditorBaseState);
  String _coordinates = "";
  ...
  void initState() {
    ...
    shapeNotifier.addListener(() {
      if (shapeNotifier.draggableShapes.isEmpty ||
          shapeNotifier.lineShapes.isEmpty) return;
      ...
      if (shapeNotifier.draggableShapes.length > 1)
        shapeNotifier.lineShapes.last.b =
            shapeNotifier.draggableShapes[1].center;
    });
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) {
      Future.delayed(Duration(seconds: 1)).then((_) {
        ...
        _currentStep.addListener(() {
          if (_currentStep.value == _curStep) return;
  ...
  void initStep() {
    if (widget.project.markers == null) return;
    ...
    _subMatrix = Matrix4.translationValues(origin.dx, origin.dy, 0.0)
      ..rotateZ(angle);
    ...
    Line mainAxis = Line.fromAngle(origin, _mAngle, _mainAxisPaint);
    ...
    shapeNotifier.addListener(() {
      Offset coords = _editorBaseState
          .globalToLocal(MediaQuery.of(context).size.center(Offset.zero));
      coords = transformOffset(Matrix4.inverted(_subMatrix), coords);
      coords = Offset(
          _editorBaseState.px2mm(coords.dx), _editorBaseState.px2mm(coords.dy));
      setState(() {
        _coordinates =
            "${coords.dx.toStringAsFixed(3)}, ${coords.dy.toStringAsFixed(3)}";
      });
    });
  ...
  void nextStep() {
    switch (_curStep) {
      ...
      case 2:
        shapeNotifier.draggableShapes.clear();
        shapeNotifier.lineShapes[1].b =
            shapeNotifier.lineShapes[0].xtersect(shapeNotifier.lineShapes[1]);
        ...
        shapeNotifier.addListener(() {
          Offset coords = _editorBaseState
              .globalToLocal(MediaQuery.of(context).size.center(Offset.zero));
          coords = transformOffset(Matrix4.inverted(_subMatrix), coords);
          coords = Offset(_editorBaseState.px2mm(coords.dx),
              _editorBaseState.px2mm(coords.dy));
          setState(() {
            _coordinates =
                "${coords.dx.toStringAsFixed(3)}, ${coords.dy.toStringAsFixed(3)}";
          });
      ...
      case 1:
        if (shapeNotifier.lineShapes.length > 1) {
          shapeNotifier.lineShapes
              .removeRange(1, shapeNotifier.lineShapes.length);
        }
        ...
        Line crossAxis = Line.perpendicular(
            shapeNotifier.lineShapes.first,
            _editorBaseState
                .globalToLocal(MediaQuery.of(context).size.center(Offset.zero)),
            _crossAxisPaint);
        ...
        shapeNotifier.draggableShapes.clear();
        shapeNotifier.draggableShapes.add(Circle(
            crossAxis.a,
            EditorBase.touchRadius / _editorBaseState.scaleFactor,
            _crossAxisPaint));
      ...
      default:
        ...
        if (shapeNotifier.lineShapes.isEmpty) {
          Line mainAxis = Line(
              _editorBaseState.globalToLocal(
                  Offset(longestSide * 1 / 4, shortestSide * 1 / 2)),
              _editorBaseState.globalToLocal(
                  Offset(longestSide * 3 / 4, shortestSide * 1 / 2)),
              _mainAxisPaint);

          shapeNotifier.lineShapes.add(mainAxis);
          shapeNotifier.draggableShapes.add(Circle(mainAxis.a,
              EditorBase.touchRadius / scaleFactor, _mainAxisPaint));
          shapeNotifier.draggableShapes.add(Circle(mainAxis.b,
              EditorBase.touchRadius / scaleFactor, _mainAxisPaint));
        } else {
          shapeNotifier.lineShapes
              .removeRange(1, shapeNotifier.lineShapes.length);
  ...
  void addMarker([Offset center]) {
    ...
    Crosshair ch = Crosshair(
        _offset,
        angle,
        transformedCenter * _editorBaseState.isoRatio,
        Crosshair.markerPaint,
        _editorBaseState.scaleFactor);
    ...
    for (i = 0; i < _markers.length; i++) {
      if (transformed[i].dx.abs() < transformedCenter.dx.abs()) break;
    }
    ...
    return Align(
        ...
        child: Column(
          ...
          children: [
            ...
            VisibleButton(
                child: Icon(
                  (_curStep < 2) ? Icons.chevron_right : Icons.add,
                  size: 56.0,
                  color: Colors.white70,
                ),
                onTap: () =>
                    (_curStep < 2) ? _currentStep.value++ : addMarker(),
                visible: _canAdd,
                keepSpace: true),
          ],
        ));
  ...
  Widget coordinates() {
    if (_curStep != 2) return Container(width: 0, height: 0);
        ...
        EditorBase(
            ...
            onRotation: (delta) {
              ...
              setState(() {
                ...
                _baseAngle =
                    _baseAngle < -pi ? _baseAngle + 2 * pi : _baseAngle;
                if (_curStep == 2)
                  _mAngle = _baseAngle + shapeNotifier.lineShapes[0].mAngle;
              });
            },
...
class MarkerUiPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect crossSection = center.translate(
            -Crosshair.strokeWidth / 2, -Crosshair.strokeWidth / 2) &
        Size(Crosshair.strokeWidth, Crosshair.strokeWidth);
