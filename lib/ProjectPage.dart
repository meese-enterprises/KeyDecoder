...
class ProjectPageState extends State<ProjectPage> {
  ...
  Widget generateTableForAxis(List<Offset> markers) {
    ...
    Matrix4 bittingMat = Matrix4.inverted(
        Matrix4.translationValues(project.originX, project.originY, 0)
          ..rotateZ(project.angle));

    for (var i = 0; i < markers.length; i++) {
      markers[i] = transformOffset(bittingMat, markers[i]);
      depths.add(markers[i].dy.abs() * project.isoRatio);
      if (i > 0)
        gaps.add((markers[i - 1].dx.abs() - markers[i].dx.abs()).abs() *
            project.isoRatio);
    }

    depths.forEach((d) {
      depthsText.add(Center(
          child: Container(
      ...
      child: Column(
        children: [
          Table(
              ...
              children: [TableRow(children: depthsText)]),
          Padding(
            padding: EdgeInsets.symmetric(
                horizontal:
                    MediaQuery.of(context).size.width / depthsText.length / 2),
            child: Table(
                border: TableBorder(
                  bottom: BorderSide(),
                  left: BorderSide(),
                ...
                children: [TableRow(children: gapsText)]),
          ),
        ],
      ),
    );
  ...
  void selectedImageSource(ImageSource src) async {
    ...
    if (project.pathRawPic.isNotEmpty) File(project.pathRawPic).deleteSync();
    ...
    EditorBase.initEditorBase().then((_) => cropRawImage().then((cropped) {
          if (cropped)
            measureCuts().then((_) {
  ...
  Future<bool> cropRawImage() async {
    final result = await Navigator.of(context)
        .pushNamed(CropPage.routeName, arguments: File(project.pathRawPic));
  ...
  Future measureCuts() async {
    await Navigator.of(context)
        .pushNamed(MeasurePage.routeName, arguments: project);
    await ProjectsDatabase.dao.updateProject(project);
...
class ProjectFloatingActionButtonState
    extends State<ProjectFloatingActionButton>
    with SingleTickerProviderStateMixin {
  ProjectPageState get parent =>
      context.findAncestorStateOfType<ProjectPageState>();
  bool isOpened = false;
  ...
  Widget cropFab({bool active = false, bool completed = false}) {
    return Container(
      child: FloatingActionButton(
        onPressed: (active || completed)
            ? () => EditorBase.initEditorBase().then((_) => parent
                .cropRawImage()
                .then((_) => EditorBase.disposeEditorBase()))
            : null,
        tooltip: "Crop image",
        ...
        Positioned(
          ...
          child: Row(
            ...
            children: [
              ...
              lineDiv(show: true),
              cropFab(
                  active: parent.project.pathCroppedPic.isEmpty &&
                      parent.project.pathRawPic.isNotEmpty,
                  completed: parent.project.pathCroppedPic.isNotEmpty),
              lineDiv(show: true),
              measureFab(
                  active: parent.project.pathCroppedPic.isNotEmpty,
                  completed: parent.project.markers != null)
            ],
          ),
        )
      ],
