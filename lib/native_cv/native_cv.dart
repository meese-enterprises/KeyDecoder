...
class Coordinate extends Struct {
  ...
  factory Coordinate.allocate(double x, double y) => allocate<Coordinate>().ref
    ..x = x
    ..y = y;

  factory Coordinate.fromOffset(Offset o) => allocate<Coordinate>().ref
    ..x = o.dx
    ..y = o.dy;
}
...
class DartIntersection {
  ...
  DartIntersection.fromList(List<Offset> l) {
    if (l.length != 4)
      throw ArgumentError(
        "DartIntersection.fromList(): Given list length != 4"
      );
...
class HomographyWarp {
  static Future<int> homographyWarp(
      DartIntersection points, String src, String dst) async {
    DynamicLibrary nativeCvLib = _getDynamicLibrary();

    var lookup = nativeCvLib
        .lookup<NativeFunction<homogaphy_warp_function>>("homography_warp");
    ...
    return homographicWarp(
      ninter.addressOf, Utf8.toUtf8(src), Utf8.toUtf8(dst)
    );
  }
